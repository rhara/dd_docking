...
**対応予定: フレキシブル受容体にはAutoDock-GPU、リジッドスクリーニングには
Vina-GPU 2.1。** Vina-GPU+（2.0）は、`dd_docking`のアンサンブルドッキングが
実際に必要とするフレキシブル側鎖のケースには使えない（上記のハード
リミテーション参照）——フレキシブル側鎖に対応したGPUドッキングを実現する
には、フレキシブル受容体側鎖をネイティブにサポートする
[AutoDock-GPU](https://github.com/ccsb-scripps/AutoDock-GPU)への切り替えが
必要になる。別軸として、リジッド受容体のスクリーニング用途（フレキシブル
残基なし）に限れば、
[Vina-GPU 2.1](https://github.com/DeltaGroupNJUPT/Vina-GPU-2.1)は現在
このプロジェクトがビルドしている2.0系列より大幅に高速だと報告されている。
どちらの統合も対応予定だが、まだ未実装——`--backend`は現時点ではCPU
QuickVina2とVina-GPU+ 2.0のみを認識する。
...
**`--n-jobs` の挙動とCPU割り当て**: `dd_docking-dock`/`screen_ensemble`が
公開しているCPU割り当てのつまみは`--n-jobs`のみであり、これとは別に
`--cpu`/`--ncpu`のようなフラグはこのCLIには存在しない。内部的には、
`--n-jobs`が各`(ligand, member)`タスクに何コアを与えるかを、qvina2自身の
（ユーザーには見えない）`--cpu`フラグ経由で制御している:`--n-jobs 1`
（デフォルト）は逐次実行し、各タスクに利用可能な全コアを与える
（`cpu_count // 1`）。それ以外の`--n-jobs`の値では、その数だけの並列
ワーカープロセス間でコアが均等に分割される（`cpu_count // n_jobs`、
最小1コア）。フレキシブル側鎖ドッキングはリジッドドッキングよりもはるかに
重く、QuickVina2自身のマルチスレッド化の恩恵を大きく受けるため、軽量な
リジッドドッキングでよく使われるパターン——各ワーカーを1コアに固定して
ジョブ側を並列化する——はここでは大きく裏目に出る。本リポジトリのERαデータ
（1リガンド・1メンバー、`--exhaustiveness 16`、16コアLinuxマシン）で実測
した、タスクあたりのコア数ごとの単一タスク実行時間:

| タスクあたりのコア数 | 実行時間 | 1コア比の高速化 |
|---|---|---|
| 1 | 827.5秒 | 1.0倍 |
| 2 | 432.4秒 | 1.9倍 |
| 4 | 223.2秒 | 3.7倍 |
...
方がマシンをうまく使える傾向にある。まずは控えめに（タスクあたり4〜8コア
程度、つまり`--n-jobs`を`cpu_count // 6`程度）始め、ライブラリ規模と
アンサンブル規模に応じて調整するとよい。
